...
proc ref photon.bounce (n, crit_angle) /* Interact with top surface */
{
...
proc ref photon.move(n, crit_angle, ref rng) /* move to next scattering or absorption event */
{
  ...
  y += d * v;
  z += d * w;
  if z<=0 then bounce(n, crit_angle);
}

proc ref photon.absorb (ref heat, bins_per_mfp, albedo, ref rng) /* Absorb light in the medium */
{
...
proc main ()
{
  ...
  on targetLoc {
    ...
    var photons: [0..<numPhotons] photon = noinit;

    @gpu.assertEligible
    forall (idx, photon) in zip(photons.domain, photons) {
      var rng: curandState_t;
      rng_init(1, idx, rng);

      photon.launch (rs);
      while photon.weight > 0 {
        photon.move (n, crit_angle, rng);
        photon.absorb (heat, bins_per_mfp, albedo, rng);
        photon.scatter (g, rng);
      }
    }
