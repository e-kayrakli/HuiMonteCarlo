...
proc ref photon.launch() /* Start the photon */
{
  x = 0.0; y = 0.0; z = 0.0;
  u = 0.0; v = 0.0; w = 1.0;
  weight = 1.0 - rs;
...
proc ref photon.bounce () /* Interact with top surface */
{
...
proc ref photon.move() /* move to next scattering or absorption event */
{
...
proc ref photon.absorb () /* Absorb light in the medium */
{
...
proc ref photon.scatter() /* Scatter photon and establish new direction */
{
...
proc print_results(rd, bit) /* Print the results */
{
  writef("%s\n%s\n\nScattering = %8.3dr/cm\nAbsorption = %8.3dr/cm\n",t1,t2,mu_s,mu_a);
  writef("Anisotropy = %8.3dr\nRefr Index = %8.3dr\nPhotons    = %8i",g,n,numPhotons);
...
proc main ()
{
  var photons: [0..<numPhotons] photon;

  forall photon in photons {
    photon.launch ();
